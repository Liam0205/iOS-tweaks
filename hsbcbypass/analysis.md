...
## Round 67（2026-08-24,★★ 方向2: dump 出检测数据 = 内存完整性校验(SKS 结构 + 哈希比对))

### hook 0x774fec(收集器)+0x7753c4(决策), dump 参数/返回缓冲
- `0x774fec` 返回缓冲 `0x100b28580`, 内容(稳定): `534b5315 6b8021f4 ... 00b0c46f 01000000 0080bc47 02000000 ...`
  - magic = **"SKS"+0x15**(Promon SHIELD 结构标识)。含运行时地址字段 0x16fc4b000 / 0x247bc8000(区域描述符)。
- `0x7753c4(w0=0x203, bufA, bufB)` 返回 **0**, bufA/bufB 内容:
  `982ed66f 01000000`(=0x16fd62e98 运行时地址)、`0c344917`(=0x1749340c)、`f1976b5a`(=0x5a6b97f1)、
  `d893c201`、`3881c101` —— **一串 32 位哈希/校验和 + 地址**。
- ⇒ ★ **检测机制 = 内存完整性校验**: Promon 收集内存区域描述(SKS 结构)+ 计算各区域哈希, 比对期望值。
  ElleKit 注入/hook 改了内存 → 哈希不匹配 → 判越狱/篡改。这就是"比对 dyld cache/内存页"的具体形态。
- 决策函数 0x7753c4 返回 0 但仍退出 ⇒ 比对结果不体现为它的返回值, 而是写入某状态(供 CFF verdict 读)。

### 现实评估(方向2 深水区)
- 完全绕过需: (a) 伪造哈希计算(让被改内存算出期望哈希), 或 (b) 对该进程隐藏内存改动(=方向3 内核层)。
